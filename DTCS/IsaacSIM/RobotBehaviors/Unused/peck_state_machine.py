...
from omni.isaac.cortex.dfb import DfLift
...
class PeckState(DfState):
    def is_near_obs(self, p):
        #print("HELPPPPPPPP ---------------------------------")
        #for _, obs in self.context.world.Obs.items():
...
def make_decider_network(robot):
    ...
    #root = DfStateMachineDecider(DfStateSequence([PeckState(), DfTimedDeciderState(DfLift(height=0.05), activity_duration=0.25)], loop=True))
    root = DfStateMachineDecider(DfStateSequence([PeckState(), DfTimedDeciderState(DfLift(height=0.05), activity_duration=1)], loop=True))
